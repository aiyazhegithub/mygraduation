...
<div class="table-responsive">
    <table class="table table-striped">
        ...
        <tbody>
        <c:forEach items="${dealTables}" var="list" varStatus="status">
            <tr >
                <td>
                    <a href="${pageContext.request.contextPath }/list/todetail?id=${list.publishGoods.id}" target="_blank">
                    <div style="float: left">
                      <img src="/${pageContext.request.contextPath }/img/${list.publishGoods.img}"
                      style="height: 110px">

                      <%--  <img
                                src="https://www.nihaostillme.site:81/imgs/${list.img}"
                                style="height: 110px">--%>
                ...
                <c:if test="${list.endDate == null}">
                    <td style="line-height: 110px" class="d${status.index}">
                        否
                    </td>
                </c:if>
                <c:if test="${list.endDate != null}">
                ...
                <c:if test="${list.endDate == null}">
                    <td>
                        <div class="btn btn-default cc" _index="${status.index}" _value="${list.id}" >确认收货</div>
                    </td>
                </c:if>
            </tr>
...
<script>
$(function () {
    $(".cc").click(function (target) {
        var conbtn=$(target.target);

        var btnindex=conbtn.attr("_index");
        var dealId=conbtn.attr("_value");
        console.log(btnindex);
        
        $.post(
            "${pageContext.request.contextPath }/deals/confirm",
            {
                dealId:dealId
            ...
            function (data,status) {
                ...
                if(status=="success"){
                    conbtn.remove();
                    $(".d"+btnindex).html("<span class='span_red'>是<span>");
                    console.log(status)
                }
